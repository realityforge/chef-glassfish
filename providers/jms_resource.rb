...
action :create do
  ...
  bash "asadmin_create-jms-resource #{new_resource.name}" do
    not_if "#{asadmin_command('list-jms-resources')} #{new_resource.target} | grep -F -x -- '#{new_resource.name}'", :timeout => 150
    timeout 150
...
action :delete do
  ...
  bash "asadmin_delete-custom-resource #{new_resource.name}" do
    only_if "#{asadmin_command('list-jms-resources')} #{new_resource.target} | grep -F -x -- '#{new_resource.name}'", :timeout => 150
    timeout 150
    user new_resource.system_user
    group new_resource.system_group
    code asadmin_command(command.join(' '))
