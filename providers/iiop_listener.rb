...
action :create do
  ...
  bash "asadmin_create-iiop-listener #{new_resource.iioplistener_id}" do
    not_if "#{asadmin_command('list-iiop-listeners')} #{new_resource.target} | grep -x -- '#{new_resource.iioplistener_id}'"
...
action :delete do
  ...
  bash "asadmin_delete_iiop-listener #{new_resource.iioplistener_id}" do
    only_if "#{asadmin_command('list-iiop-listeners')} #{new_resource.target} | grep -x -- '#{new_resource.iioplistener_id}'"
    user new_resource.system_user
    group new_resource.system_group
    code asadmin_command(command.join(' '))
  end
