...
action :create do
  ...
  execute "asadmin_create-managed-scheduled-executor-service #{new_resource.jndi_name}" do
    ...
    not_if "#{asadmin_command('list-managed-scheduled-executor-services')} #{new_resource.target} | #{filter}", :timeout => 150
  end
...
action :delete do
  ...
  bash "asadmin_delete-managed-scheduled-executor-service #{new_resource.jndi_name}" do
    ...
    filter = pipe_filter(new_resource.jndi_name, regexp: false, line: true)
    only_if "#{asadmin_command('list-managed-scheduled-executor-services')} #{new_resource.target} | #{filter}", :timeout => 150
  end
end
